...
public class espDevice implements Runnable{
    ...
    private ArrayList<DeviceFunctionDefinition> definedFuncs = new ArrayList<>();
    ...
    public boolean handShake() throws IOException, Exception  {
    	...
    	if (shake == 12){
			...
			for (int i = 0; i < rawFuncs.length; i++){
				...
				if(DefinitionLibrary.getDevFunc(new Integer(rawFuncs[i])) == null){
					definedFuncs.add(new DeviceFunctionDefinition((byte)0, "Undefined, please update or contact dev"));
				} else{
					definedFuncs.add(DefinitionLibrary.getDevFunc(new Integer(rawFuncs[i])));
				}
    ...
    public ArrayList<DeviceFunctionDefinition> getDefinedFuncs(){
    	return this.definedFuncs;
    }
